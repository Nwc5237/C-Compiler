...
{
    / *
     * Write a test suite here. The best idea is probably to just make a few
     * lines of random code to test, and then try reading actual c files and
     * seeing if it outputs all of the token of the program correctly. After
     * that try passing files with lexical errors and see how you can handle
     * them.
     * /
...
printf("input=%s\n", input);
    while(1)
    {
        //scan returns 1 when it finds a token and 0 on error
        result = scan(&pos);
        if(result)
